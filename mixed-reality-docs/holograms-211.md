...
[ジェスチャ](gestures.md)によって、ユーザーの意図が動作するようになります。 ジェスチャを使用すると、ユーザーはホログラムを操作できます。 このコースでは、ユーザーの手を追跡し、ユーザー入力に応答して、手動による状態と場所に基づいてユーザーにフィードバックを送る方法について説明します。
...
2. 右側に **+** あるをクリックして、新しいキーワードを追加します。
3. 「 **Move Astronaut**」というキーワードを入力します。 必要に応じて、キーのショートカットを自由に追加できます。
4. 右側に **+** あるをクリックして、新しいキーワードを追加します。
...
* 注:手を動かす前に "Move Astronaut" と言うと、代わりにナビゲーションジェスチャが使用されます。
...
9. ボタンをクリックし、[キーワード] ドロップダウンから [モデルの展開] を選択します。 **+**
10. [応答] の下をクリックし、 **AstronautManager**を**階層**から [**なし] (オブジェクト)** フィールドにドラッグします。 **+**
11. 次に、[**関数なし**] ドロップダウンをクリックし、[ **AstronautManager**]、[ **expandmodelcommand**] の順に選択します。
12. 音声入力ハンドラーの **+** ボタンをクリックし、[キーワード] ドロップダウンから [モデルの**リセット**] を選択します。
13. [応答] の下をクリックし、 **AstronautManager**を**階層**から [**なし] (オブジェクト)** フィールドにドラッグします。 **+**
...
おめでとうございます! これで MR 入力**211 が完了しました。ジェスチャ**。
