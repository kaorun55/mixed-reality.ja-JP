...
* **Virtual Reality sdk**の一覧に [ **Windows Holographic** ] が表示されていることを確認します。 表示されてい **+** ない場合は、一覧の下部にあるボタンを選択し、[ **Windows Holographic**] を選択します。
* [**発行の設定**] を展開します。
...
* 新しいシーンを保存します。**ファイル > シーンに名前を付けて保存...**
...
* [**アクティブなプロファイラー <Enter IP>の >** ] をクリックします。
...
* 起動中 **()** 関数を見つけて、次のコード行を追加します。**TrianglesPerCubicMeter = 1200;**
...
* [移動 **] オブジェクトを**[**階層**] パネルにドラッグアンドドロップします。
...
配置可能**なスクリプトは**、平面検索が完了した後に作成されたポスターとプロジェクションボックスに既にアタッチされています。 いくつかのコードをコメント解除するだけで、次のことが実現されます。
...
おめでとうございます! これで MR 空間**230 が完成しました。空間マッピング**。
