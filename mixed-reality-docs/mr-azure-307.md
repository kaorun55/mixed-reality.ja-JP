...
3.  左側のパネルから展開**保存されたデータセット** > **My Datasets**をドラッグし、 **ProductsTableCSV**に、**実験キャンバス**.
...
7.  **実験**左側のパネルで、展開**Machine Learning** > **トレーニング**します。 ドラッグ、**モデルのトレーニング**を実験キャンバスにアウト項目。 キャンバスは同じになりますの下。
...
12. 左側の 実験項目パネルから展開**Machine Learning** > **モデルの初期化** > **分類**、をドラッグします。**多クラス ロジスティック回帰**を実験キャンバス上に項目。

13. 一番下から、出力を接続、**多クラス ロジスティック回帰**、左の入力に、**モデルのトレーニング**項目。

    ![Machine Learning Studio:実験](images/AzureLabs-Lab7-23.png)

14. 左側のパネルの 実験項目の一覧で展開**Machine Learning** > **スコア**、ドラッグ、**モデルのスコア付け**をキャンバス上に項目。
...
17. 一覧で**実験**、左側のパネルでアイテムの展開**Machine Learning** > **Evaluate**、ドラッグ、 **モデルの評価**をキャンバスに項目。
...
3.  次に移動**ファイル** > **Build Settings**にプラットフォームを切り替えると**ユニバーサル Windows プラットフォーム**、 をクリックして、***プラットフォームの切り替え***ボタンをクリックします。
...
15. シーンとプロジェクトを保存**ファイル** > **シーン/ファイルの保存** > **プロジェクトの保存**します。
...
3.  チェック ボックスと呼ばれる**UnityC#プロジェクト**(これは重要なビルドが完了した後に、クラスを編集することを許可することがあるため)。
