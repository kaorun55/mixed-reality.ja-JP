...
3.  左側のパネルから展開 **保存されたデータセット* > *My Datasets** をドラッグし、 **ProductsTableCSV**に、**実験キャンバス**します。
...
7.  **実験**左側のパネルで、展開 **Machine Learning* > *トレーニングします**。 ドラッグ、**モデルのトレーニング**を実験キャンバスにアウト項目。 キャンバスは同じになりますの下。
...
12. 左側の実験項目パネルでは、次のように展開します ***Machine Learning* > *モデルの初期化* > *分類***、およびドラッグ、 **多クラス ロジスティック回帰。** を実験キャンバス上に項目。
...
16. 右下の出力を接続 **データの分割** 、右の入力に、 **モデルのスコア付け* 項目*します。

    ![Machine Learning Studio:実験](images/AzureLabs-Lab7-24.png)

17. 一覧で**実験**、左側のパネルでアイテムの展開 ***Machine Learning* > *評価***、およびドラッグ、 **モデルの評価** をキャンバスに項目。
